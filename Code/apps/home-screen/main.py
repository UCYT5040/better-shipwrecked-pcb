...
class App(badge.BaseApp):
    ...
    def render_home_screen(self):
        ...
        apps = self.get_apps_to_show()
        current_screen = self.cursor_pos // 6
        start = current_screen * 6
        end = min(start + 6, len(apps))
        for i in range(start, end):
            app = apps[i]
            app_x = ((i - start) % 3) * 66
            app_y = ((i - start) // 3) * 78 + 12
            self.draw_app_icon(app, app_x, app_y, self.cursor_pos == i)
        self.logger.debug(f"Cursor position: {self.cursor_pos}, total apps: {len(apps)}")
        badge.display.show()

    def get_apps_to_show(self):
        apps = filter(lambda app: app.app_path != "/apps/home-screen", internal_os.apps.registered_apps)
